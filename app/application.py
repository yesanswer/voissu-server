...
class Application:
    ...
    def exit_user(self, user):
        if user.uid not in self.users or self.users[user.uid].guid != user.guid:
            return

        print('exit_user - {}'.format(user.uid))

        if user.owner_channel:
            print('user joined channel : {}'.format(user.owner_channel.id))
